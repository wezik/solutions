...
public class ObservedPin {
    ...
    private static final String[] possibilities = new String[] {
        "08",
        "124","2135","326",
        "4157","52468","6359",
        "748","85790","986"
    ...
    public static List<String> getPINs(String observed) {
        ...
        generatePin(0, new StringBuilder(), observed, resultList);
        return resultList;
    }

    private static void generatePin(int i, StringBuilder buffer, String observed, List<String> resultList) {
        if (i == observed.length()) {
            resultList.add(buffer.toString());
            return;
        }
        String possibleNumbers = possibilities[Character.getNumericValue(observed.charAt(i))];
        for (char c : possibleNumbers.toCharArray()) {
            buffer.append(c);
            generatePin(i+1 , buffer, observed, resultList);
